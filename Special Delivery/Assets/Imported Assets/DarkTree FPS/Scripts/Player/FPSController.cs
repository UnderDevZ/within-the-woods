...
namespace DarkTreeFPS
{
	public class FPSController : MonoBehaviour
	{
		...
		[Tooltip("Time the player can run for")]
		public float runLength;
		[Tooltip("Time until player can run again")]
		...
		private void Start()
		{
			...
			runTimer = runLength;
			slider.maxValue = runLength;
			slider.value = runLength;
		}
		...
		void StandaloneMovement()
		{
			if (isGrounded())
			{
				...
				if (Input.GetKey(inputManager.Run) && !isClimbing && !crouch && weaponHolderAnimator.GetBool("Walk") == true && canRun) {
